...
from lean_trees.core.lean_file import LeanTheorem
from lean_trees.core.project import LeanProject
from lean_trees.repl_adapter.interaction import LeanProcessException, LeanServer
from lean_trees.utils import Logger, LogLevel
...
def create_environment(project: LeanProject, repl_path: Path, header: str) -> Any:
    ...
    env = project.environment(repl_path, Logger(LogLevel.INFO)).__enter__()
    if header:
...
def main():
    ...
    project = LeanProject(args.project_path)
    ...
    def restart_env():
        ...
        env = create_environment(project, args.repl_path, sample["header"])
