...
from lean_trees.core.abstraction import ProofBranch
from lean_trees.core.lean import LeanGoal, LeanTactic, LeanProofState
from lean_trees.core.lean_file import LeanTheorem, LeanFile, StoredError
from lean_trees.file_span import FilePosition, FileSpan
from lean_trees.metavar_graph import MetavarGraph
from lean_trees.repl_adapter.data import ReplGoalInfo, ReplCompilationUnit, FilePositionParser, ReplProofStepInfo
from lean_trees.utils import is_just_comments, ValueOrError, get_source_with_sorries, to_sync, to_sync_iterator
...
# TODO: to speed it up:
#   - when sending auxiliary tactics like rotate_left and sorry, switch off mvar graph report
...
class LeanServer:
    ...
    async def pickle_async(self, path: Path) -> PickledEnv:
        ...
        return PickledEnv(path)

    pickle = to_sync(pickle_async)

    async def unpickle_async(self, pickled_env: PickledEnv):
        """Unpickle a REPL environment from a file asynchronously."""
        self._assert_started()
        response = await self._send_to_repl_async({"unpickleEnvFrom": str(pickled_env.path)})
    ...
    async def start_proofs_from_sorries_async(self, theorem_with_sorries: str) -> "AsyncIterator[LeanProofBranch]":
    ...
    start_proofs_from_sorries = to_sync_iterator(start_proofs_from_sorries_async)

    async def start_proof_from_sorry_async(self, theorem_with_sorry: str) -> "LeanProofBranch":
        """Start a proof from a sorry asynchronously."""
        proofs = [branch async for branch in self.start_proofs_from_sorries_async(theorem_with_sorry)]
        if len(proofs) != 1:
            raise Exception(f"{len(proofs)} occurrences of `sorry` in the theorem (expected 1).")
        return proofs[0]

    start_proof_from_sorry = to_sync(start_proof_from_sorry_async)

    async def async_start_file_proofs(self, file: LeanFile) -> "AsyncIterator[tuple[LeanTheorem, list[LeanProofBranch] | Exception]]":
        """Start file proofs asynchronously."""
        async for unit, sorry_branches in self.async_start_runnable_proofs(RunnableFile.from_lean_file(file)):
    ...
    start_file_proofs = to_sync_iterator(async_start_file_proofs)

    async def async_start_runnable_proofs(
            self,
            file: RunnableFile,
    ...
    start_runnable_proofs = to_sync_iterator(async_start_runnable_proofs)

    async def start_full_proofs_async(self, file: LeanFile) -> "AsyncIterator[tuple[LeanTheorem, LeanProofBranch]]":
...
class ReplProofStatusType(Enum):
    # Proof is complete and type checks successfully.
    Completed = 0
    # When goals remain, or proof contains sorry/metavariables.
    Incomplete = 1
    # When kernel type checking errors occur.
    Error = 2


@dataclass
class ReplProofStatus:
    type: ReplProofStatusType
    full_message: str
...
class LeanProofBranch(ProofBranch[LeanGoal, LeanTactic]):
    ...
    async def try_apply_tactic_async(self, tactic: LeanTactic | str) -> ValueOrError[list[Self]]:
        try:
            return ValueOrError.success(await self.apply_tactic_async(tactic))
    ...
    @classmethod
    def _check_tactic(cls, tactic: str):
        tactic = tactic.strip()
    ...
    @classmethod
    def proof_status_from_response(cls, response: dict) -> ReplProofStatus:
        status_str = response["proofStatus"]
        if status_str.startswith("Completed"):
            status_type = ReplProofStatusType.Completed
        elif status_str.startswith("Incomplete"):
            status_type = ReplProofStatusType.Incomplete
        elif status_str.startswith("Error"):
            status_type = ReplProofStatusType.Error
        else:
            raise Exception(f"Unknown proof status: '{status_str}'")

        return ReplProofStatus(status_type, status_str)
