...
import lean_trees.file_span
import lean_trees.utils
from lean_trees.repl_adapter.data import ReplCompilationUnit, ReplLoadedLeanFile, ReplProofStepInfo, FilePositionParser
...
class LeanFileParser:
    ...
    def load_lean_file(
        ...
        with open(path) as f:
            file_line_lengths = [len(line) for line in f.readlines()]
    ...
    def load_compilation_units(
        ...
        for edges, root_info_tree in zip(data["proof_tree_edges"], data["info_trees"]):
            ...
            if pretty_print:
                pretty_print = lean_trees.utils.remove_empty_lines(lean_trees.utils.remove_comments(pretty_print))
