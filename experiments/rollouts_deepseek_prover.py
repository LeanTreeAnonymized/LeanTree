...
# TODO: (maybe) ban repeating the same tactic twice in a row

# TODO!!: after REPL error, we should try to restart it (3 times?) before cutting the rollout
...
def get_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--checkpoint", type=str, default="deepseek-ai/DeepSeek-Prover-V2-7B")
    ...
    parser.add_argument("--output_dir", type=Path, default="rollouts_deepseek")
...
ARGS_WHITELIST = [
    ...
    "max_thinking_length", "max_tactic_length", "temperature", "repetition_penalty"
]
...
class Logger:
    def __init__(self, log_dir: Path):
        ...
        print(f"model_outputs_path: {self._model_outputs_path}")
        print(f"rollout_steps_path: {self._rollout_steps_path}")
        print(f"incomplete_rollouts_path: {self._incomplete_rollouts_path}")
        print(f"exceptions_path: {self._exceptions_path}")
        print(f"stats_path: {self._stats_path}")
        print(f"final_proofs_path: {self._final_proofs_path}")
    ...
    def _calculate_stats(self, completed_rollouts: "list[RolloutProofSearch]") -> dict:
        ...
        return {
            ...
            "proven_rate": len([r for r in completed_rollouts if r.proven]) / len(completed_rollouts) if completed_rollouts else 0,
            ...
            "failed_rollouts_rate": sum([r.failed_rollouts for r in completed_rollouts]) / total_rollouts if total_rollouts > 0 else 0,
            "total_failed_searches": sum([r.failed for r in completed_rollouts]),
            "failed_searches_rate": sum([r.failed for r in completed_rollouts]) / len(completed_rollouts) if completed_rollouts else 0,
...
Suggest a next tactic to use in the proof that user provides.
Only respond with the tactic enclosed in <tactic> </tactic> tags, nothing else.
...
class RolloutProofSearch:
    ...
    async def _next_prompt(self) -> str:
        ...
        #         message = f"""
        # Suggest the next tactic.
        # Only respond with the tactic enclosed in <tactic> </tactic> tags, nothing else.
        # Only output one Lean tactic (no informal text, no nested tactics, no comments, no sorry keyword).
        #         """.strip()
        message = "Suggest the next tactic."
        ...
        if self.args.method == "whitebox":
            ...
            theorem_str = goal_str
        else:
            assert self.args.method in ["graybox", "blackbox"]
            theorem_str = (
                self.theorem[:-len("sorry")] +
                ("\n" if len(self.proof) > 0 else "") +
                "\n".join([f"  {tactic}" for tactic in self.proof])
            )
        return f"{message}\n\n```lean4\n{theorem_str}\n```"
                ...
                if rollout.repl_restarts >= 10:
                    ...
                    if rollout.repl_restarts_in_step >= 3:
                        ...
                        logger.log_incomplete_rollout(
                            rollout._get_proof_str(include_sorry=False), f"REPL restarted too many times, again got exception {note}: {error}"
                        )
