...
from lean_trees.repl_adapter.interaction import LeanServer, LeanInteractionException, LeanProcessException
...
def get_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--max_output_tokens", type=int, default=2560)

    parser.add_argument("--benchmark_cache_dir", type=Path, default="benchmarks")
    parser.add_argument("--output_dir", type=Path, default="whole_proof_generation")
...
class Logger:
    ...
    def log_error_proof(self, unit: str, error: LeanInteractionException):
...
class Model:
    ...
    def generate(self, statement: str) -> list[str]:
        messages = [[
            {"role": "user", "content": prompt.format(statement)},
        ]]
        ...
        return ["```lean" + proof for proof in output_decoded]
...
class Verifier:
    ...
    async def verify(self, theorem: str, proof: str) -> tuple[ProofResult, str, LeanInteractionException | None]:
        while True:
    ...
    async def _restart_repl(self):
        if self.repl:
            ...
            self.repl = None
        self.repl = LeanServer(self.args.repl_exe, self.args.project_path)
